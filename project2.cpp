...
void colour(adjMatrix const &adj, int colour1, int colour2, int pessoa, vector<int> &colours, vector<int> &counts);
void count(adjMatrix const &adj, vector<int> &counts, int pessoa);
...
int main(){
    ...
    colour(adjTransposed, WHITE, BLUE, v1, colours, counts);
    colour(adjTransposed, BLUE, RED, v2, colours, counts);
...
int adicionarMatrizAdj(adjMatrix &adj, int pessoa, int pai) {
    ...
    if(num_pais < 2) {
        adj[pessoa-1][num_pais] = pai;
        adj[pessoa-1][2] = num_pais + 1;
    }
    else
        return -1;
